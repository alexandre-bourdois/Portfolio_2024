          ...
          <TabsList className="flex md:flex-col h-full bg-transparent md:w-64 gap-4">
            {Object.keys(resumeContent.tabMenu).map((key) => (
              <TabsTrigger
                key={key}
                value={key}
                className="bg-white/10 w-full py-3 text-white data-[state=active]:bg-hoverColor 
                  hover:bg-lightSky/40 text-xs sm:text-sm "
              >
                <div className="flex items-center gap-1.5 md:w-[50%] md:gap-3">
                  {React.createElement(iconMap[key], { className: "w-4 h-4 md:h-5 md:w-5" })}
          ...
          <div className="flex-1 min-h-[400px]">
            ...
            <TabsContent value="experience">
              ...
              <div className="space-y-6">
                {resumeContent.experience.items.map((item: any, index: number) => (
                  <motion.div
                    key={index}
            ...
            <TabsContent value="education">
              ...
              <div className="space-y-6">
                {resumeContent.education.items.map((item: any, index: number) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, y: 40 }}
                    animate={{ opacity: 1, y: 0 }}
            ...
            <TabsContent value="about">
              ...
              <div className="space-y-6">
                <motion.div
                  ...
                  className="border rounded-lg border-lightSky p-6"
                >
                  <p className="text-white mb-6 text-lg">{resumeContent.about.bio}</p>
                  <div className="space-y-4">
                    <div>
                      <h3 className="text-lg font-semibold mb-2">Interests</h3>
                      <div className="flex flex-wrap gap-2 px-2">
                        {resumeContent.about.interests.map((interest: string, i: number) => (
                          <Badge key={i} variant="secondary">
                            {interest}
                          </Badge>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 px-2">
                        {resumeContent.about.languages.map((language: string, i: number) => (
                          <Badge key={i} variant="secondary">
                            {language}
                          </Badge>
                        ))}
